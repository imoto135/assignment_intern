"""
XGBoost model training script for time series forecasting on ETTh1 dataset.
...
OUTPUT_DIR = Path('experiments/xgboost')
DATASET = 'ETTh1'
TARGET_COL = 'OT'
...
def save_model_and_results(model, metrics, feature_columns):
    ...
    # Save model
    model_path = OUTPUT_DIR / f'xgboost_{DATASET}_{timestamp}.json'
...
def main():
    ...
    # Initialize wandb
    run_name = f"xgboost_{DATASET}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Train model
    model, evals_result = train_model(X_train, y_train, X_val, y_val)
